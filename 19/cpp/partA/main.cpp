...
int main(int argc, char* argv[])
{
    ...
    CoordinateSet allBeacons;

    for(const auto& entry : scanners)
    {
        const Scanner& scan = entry.second;

        std::cout << "Scanner " << scan.id << std::endl;
        for(const auto& beacon : scan.beacons)
        {
            allBeacons.emplace(beacon);
        }
    }
    std::cout << allBeacons.size() << std::endl;
