...
int main(int argc, char* argv[])
{
    ...
    {
        elements.emplace_back(readElement(infile));
    ...
    int max = 0;
    for(const ElementPtr& lhs : elements)
    {
        for(const ElementPtr& rhs : elements)
        {
            int magn = add(lhs, rhs)->magnitude();
            if(magn > max)
            {
                std::cout << max << " --> " << magn << std::endl;
                max = magn;
            }
        }
    }
